...
- **提示词缓存（省费用，不改输出）：** 请求按「稳定前缀在前」组织，让供应商的 prompt cache 尽量命中——① system 消息跨页逐字节一致（角色 + 工作方法 + 风格 + 禁用格式，页级内容全部移出）；② user 消息开头放**跨页共享**的仓库证据（仓库地图 / 声明依赖 / 真实命令 / 模块 / wiki 目录），页内 scope 与符号骨架排在其后；③ 页级写作要求（brief + 页头）放在最靠近生成点的末尾；④ **深度重写的请求 = 首稿的 `system` + `user` 原样前缀再追加「缺口清单 + 草稿」**，所以一页的第二遍只付未缓存的那一小段。`prompt_salt` 覆盖整段前缀（system + user），改文案仍会失效复用。命中率取自供应商上报的 `usage.prompt_tokens_details.cached_tokens`（流式与非流式都解析），累计在 `LlmClient`（每次 run 一个实例），汇总行报「提示词缓存：X/Y prompt tokens 命中（Z%）」并写入 run notes；供应商不上报该字段时第二项为 0。见 `prompt.rs` 的 `static_system` / `page_user` 与 `evidence.rs` 的 shared-first 顺序。
- **模型假死（重复调用 / 重复正文）的恢复阶梯：** 一轮既没有新正文、也没有新调用时**不直接截断**，而是往对话里注入纠偏提示（同一调用不重复执行的说明 + 「换文件/换搜索词/继续写」），最多 2 次；第二次是「停止工具调用，立即输出正文」。只有纠偏无效才结束本页（warn 出轮次）。提示词随下一次请求一起发出，不额外增加调用。
...
- **一轮里的批量调用要一次答完（否则预算被拆轮吃掉）：** 提示词要求模型「batch every read_file / grep call into ONE round」，所以同一轮请求的调用会**逐一**执行到预算上限，超出部分明确答复「预算已用尽」而不是被丢掉。曾经每轮只答前 3 个，把一批拆成多轮——每轮都要重发整个对话，拆轮比一次答完更贵；而多出的轮次还按「只读不写」计费，于是 6 个文件的批量读完两轮就耗尽默认的 6 次额度，页面还没开写就被判成卡死并答复「直接写正文」（线上表现为多个页面同一秒齐刷刷 `工具调用停止`）。回归测试 `a_batched_round_reads_every_file_it_asked_for`（一批 6 个只花 2 轮）与 `a_batch_beyond_the_budget_is_answered_not_dropped`（超预算的调用仍有对应 tool 消息）。
- **工具结果缓存（两级）：** ① *调用级*：键 = 工具名 + 规范化参数（`Value` 序列化天然按键排序），同一调用第二次直接返回；② **文件级快照**：`read_file` 每个文件一次运行只读一次盘，之后**任意行范围**都从快照切片（窗口平移、范围重叠、二次精读都命中，调用级缓存对这种"同文件不同范围"无能为力）。快照单文件 ≤1 MiB、总量 ≤16 MiB，超限则照常读盘但不驻留；run 内仓库视为冻结（run 持锁且指纹基于工作区）。失败不入缓存。汇总行报 `工具读取缓存：命中 X/Y（Z%）· 缓存 N · 文件快照 S 个（其中 R 次服务了另一个行范围）`。
- **工具输出压缩管道（语言无关 + 一道语言闸门）：** 作用于**喂给模型**的工具文本（源码窗口 / grep / 目录树），不触碰仓库文件；`read_file` 的行号栏仍按原始行号生成，所以 `path:line` 引用不受影响。① **空白归一**（CRLF、行尾空白）② **空行折叠**（连续 ≥3 空行留 1 个、去首尾空行）③ **按块缩进外提**（唯一语言相关的一步）④ **重复行折叠**（连续 ≥4 行完全相同 → 保留首行 + `(xN)`）。汇总行报 `工具输出压缩：省下 N 字符（≈N/4 tokens）`。
- **①② 与 ③④ 走的是两条管道：** `read_file` 的每一行都带原始行号，折叠空行会让行号错位，所以它走「逐行 `clip` → 缩进外提 → 重复行折叠」；其余工具输出是自由文本，走 `compact`（行尾归一 + 空行折叠）。
- **③ 缩进外提的三个设计点（都有实测支撑）：**
  1. **按块、不按窗口**：块 = 从某行起每个非空行缩进都 ≥4 空格、直到遇到更浅的为止（空行不打断、不计行数），块内最小缩进就是省掉的列数；块首尾各一行**不占行号**的记号（`// begin: 8 spaces omitted per line` / `// end`），补回列数即**逐字节复原**。实测本仓 85 万字符（rs/ts/tsx/json）：整窗外提只省 2.6%（真实读窗口常从 0 缩进行开始），按块外提省 **6.2%**（52k 字符 ≈ 13k tokens）；对照实验里"按等缩进段分块"只省 0.9%（真实代码每层往往不足 8 行），所以采用按最小缩进成块。
  2. **语言闸门是白名单**：只有 `HOIST_LANGUAGES` 表里的扩展名才外提，不在表里的一律跳过——用白名单而不是黑名单，因为漏压只是少省几个字符，误压会把写进页面的代码改错；缩进即语法的语言（python/yaml/markdown/reST/sass/pug/hs/fs/nim/txt/csv…）不在表里，所以永不外提。表内语言再用 `string_lines` 逐行拦下跨行字符串（Go 反引号原生字符串、JS/TS 模板字符串、Java/C# 三引号）——那些行的行首空格是**字符串内容**，外提即改数据。
  3. **记号自解释、不要图例**：实测图例（约 110 字符/窗口）会吃掉约三分之一收益（块均 1.3 个/窗口），所以把说明写进记号本身。
- **记账单位是字符不是字节**：中文一字 3 字节、约 1 token，用字节会让"省下多少"虚高约 3 倍——这种度量没有意义。`Compacted` 与记号成本一律按 `chars().count()`。
- **为什么不做语言专属（AST/语义）压缩**：语言无关的部分（行号栏、缩进、空白）才是无损的大头——单行号栏就占全仓字符的 20%（固定 `{:>5}` 收窄到实际宽度后省 5.8%）。再往下省必须动 import/注释/空行这些**承载信息**的内容（实验：连续 ≥4 行完全相同的代码在 rs/ts/tsx/js/json/toml/md 里**一次都没出现**，所以重复行折叠对手写代码几乎无效；它保留下来是因为**生成代码**里很值——protobuf 生成物里连续 8 行的 `_ = fileDescriptor` 折成一行 `(x8)`）；真要做 AST 级压缩就只能是有损摘要，而页面要的是可引用的原文，所以停在无损边界。
...
- 已验证（工具输出压缩，本轮 + 多语言复验）：`tools/compress.rs` 13 单测（空白归一 / 空行折叠 / **块外提精确可逆**（补回列数逐字节复原）/ 浅缩进与短块不外提 / **白名单外的语言永不外提** / **常见花括号语言都在白名单里** / **跨行字符串守卫**（`multiline_strings_are_excluded_from_blocks`、`template_literals_are_excluded_too`）/ **重复行折叠可逆**（`generated_repetition_is_folded_reversibly`）/ 短 run 不折 / 语言无关的两种变换对任何语言成立 / **真实窗口收益 ≥5% 底线**）+ `tools::tests::hoisted_windows_keep_their_line_numbers`（外提后第 N 行仍是第 N 行、记号不占行号、Python 同文本不外提）+ `tools::tests::hoist_markers_bracket_exactly_the_stripped_lines`（**记号必须夹住它标注的行**，区间读同样成立）。端到端（假供应商，同一段深层嵌套的 `.rs` 与 `.py`）：`tool output compression: 1237 characters kept out of the conversation`，Rust 结果带 `// begin: 8 spaces omitted` 记号、Python 结果逐行原样。真实代码实测（本仓 85 万字符 rs/ts/tsx/json）：按块外提省 52,358 字符（**6.2%**）、227/258 个窗口命中；对照实验：整窗外提 2.6%、按等缩进段分块 0.9%——数据选了按最小缩进成块。**多语言端到端复验**（假供应商 + 六种语言夹具，抓取客户端实际发出的 tool 结果）：Go 块外提生效且反引号里的 SQL 缩进逐字保留、protobuf 生成物连续 8 行折成 `4| _ = fileDescriptor  (x8)`（行号跳 4→12）、C++ 外提保留嵌套相对深度、proto 记号正确夹住第 5–13 行、Python 全不压、JS 模板串内部空格原样保留。
- 已验证（工具缓存 + 防死循环，本轮）：`atlas-core/src/tools/cache.rs`（6 单测：命中计数 / 键隔离 / 超大条目不缓存 / 覆写不重复计字节 / 单位格式 / **文件快照服务任意后续范围**）+ `tools::tests::identical_calls_hit_the_snapshot_cache`（同调用命中、参数键序无关命中、改盘上内容运行期内仍是快照、**换行范围走快照而不是重读盘**、失败与非法 JSON 不入缓存）。端到端（假供应商，60 行深层嵌套样本）：`tool cache: 48/51 lookups hit (94%), 2 file snapshots (1 reused for another range)`，17 页 0 模板回退。
- 已验证（工具缓存 + 防死循环）：`atlas-llm/tests/text_tool_dialect.rs` 11 用例覆盖：跨轮 id 唯一、边写边读正文只留一份、续写跨轮保序、**重复调用只执行一次**、**假死先自救**（`a_stuck_model_recovers_after_the_nudge`：模型只在看到纠偏提示后才写正文，3 个请求内完成）、纠偏无效才收尾（`repeated_identical_rounds_stop_the_loop`：1 次执行 + 2 次纠偏 + 1 轮收尾）、只读不写恰好用满预算后 2 次拒绝（8 个请求）、边写边读的读取不计费（6 段 6 次执行）、无限写读在第 12 次调用（`1×3×4`）收敛。
- 已验证（提示词缓存布局，本轮）：`prompt.rs` 单测 `system_prompt_is_identical_across_pages`（同一 run 内每页 system 逐字节一致，换输出语言才变）、`page_user_puts_instructions_last`（证据在前、页级要求在后）、`prompts_carry_outline_evidence_and_gaps` 断言 `expand.system == generate.system` 且 `expand.user.starts_with(generate.user)`（深度重写复用首稿整段前缀）；`pipeline::tests` 的 evidence 顺序断言改为 shared-first。端到端：假供应商按「与历史请求的最长公共前缀」上报 `cached_tokens`，17 页（`depth_pass=true`、并发 1）跑 `atlas init` 得 `prompt cache: 54939/102145 prompt tokens served from cache (54%)`，逐请求可见「首稿 0 命中 → 同页深度重写 1908/3065 命中 → 下一页首稿 1221/2630 命中」，汇总行「提示词缓存：…（54%）」。
