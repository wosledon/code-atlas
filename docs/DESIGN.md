...
- **工具输出压缩管道（4 种，逐个标注可逆性）：** 作用于**喂给模型**的工具文本（源码窗口 / grep / 目录树），不触碰仓库文件；`read_file` 的行号栏仍按原始行号生成，所以 `path:line` 引用不受影响。① **空白归一**（CRLF、行尾空白）——除行尾空白外逐字节相同；② **空行折叠**（连续 ≥3 空行留 1 个、去首尾空行）——空行个数不是信息；③ **公共缩进外提**（整块 ≥8 行且公共缩进 ≥4 空格时，缩进只在头部声明一次，正文按声明列数缩回即可**精确复原**）；④ **重复行折叠**（连续 ≥4 行完全相同 → 保留首行 + `⟪同上 ×N⟫`，计数即被折行数，**精确可逆**；空行不折）。汇总行报 `工具输出压缩：省下 N 字符（≈N/4 tokens）`。
...
- 已验证（工具缓存 + 防死循环，本轮）：`atlas-core/src/tools/cache.rs`（6 单测：命中计数 / 键隔离 / 超大条目不缓存 / 覆写不重复计字节 / 单位格式 / **文件快照服务任意后续范围**）+ `tools::tests::identical_calls_hit_the_snapshot_cache`（同调用命中、参数键序无关命中、改盘上内容运行期内仍是快照、**换行范围走快照而不是重读盘**、失败与非法 JSON 不入缓存）；`tools/compress.rs`（7 单测：空白归一 / 空行折叠 / **缩进外提精确可逆** / 浅缩进与短块不外提 / **重复行折叠可逆且计数正确** / 空行不折 / 管道整体变小且正文行不丢）。端到端（假供应商，60 行深层嵌套 + 15 行重复 `}` 的样本）：`tool cache: 48/51 lookups hit (94%), 2 file snapshots (1 reused for another range)`、`tool output compression: 1273 characters kept out of the conversation`，17 页 0 模板回退。
