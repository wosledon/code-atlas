...
- **工具输出压缩管道（语言无关 + 一道语言闸门）：** 作用于**喂给模型**的工具文本（源码窗口 / grep / 目录树），不触碰仓库文件；`read_file` 的行号栏仍按原始行号生成，所以 `path:line` 引用不受影响。① **空白归一**（CRLF、行尾空白）② **空行折叠**（连续 ≥3 空行留 1 个、去首尾空行）③ **按块缩进外提**（唯一语言相关的一步）。汇总行报 `工具输出压缩：省下 N 字符（≈N/4 tokens）`。
- **③ 缩进外提的三个设计点（都有实测支撑）：**
  ...
  2. **语言闸门**：缩进即语法的语言（python/yaml/markdown/reST/sass/pug/hs/fs/nim/txt/csv…）整段跳过——外提后模型无从知道真实层级，写进页面的代码就是错的；JS 家族用**反引号奇偶守卫**、TOML/INI 用三引号奇偶守卫拦下切在多行字符串中间的窗口（那里的行首空格是字符串内容）。
...
- **为什么不做语言专属（AST/语义）压缩**：语言无关的部分（行号栏、缩进、空白）才是无损的大头——单行号栏就占全仓字符的 20%（固定 `{:>5}` 收窄到实际宽度后省 5.8%）。再往下省必须动 import/注释/空行这些**承载信息**的内容（实验：连续 ≥4 行完全相同的代码在 rs/ts/tsx/js/json/toml/md 里**一次都没出现**，所以"重复行折叠"已删除）；真要做 AST 级压缩就只能是有损摘要，而页面要的是可引用的原文，所以停在无损边界。
...
- 已验证（工具输出压缩，本轮）：`tools/compress.rs` 9 单测（空白归一 / 空行折叠 / **块外提精确可逆**（补回列数逐字节复原）/ 浅缩进与短块不外提 / **语言闸门拦住 py·yaml·md·rst·sass·pug·hs·fs·txt·csv** / **反引号与三引号奇偶守卫** / 语言无关的两种变换对任何语言成立 / **真实窗口收益 ≥5% 底线**）+ `tools::tests::hoisted_windows_keep_their_line_numbers`（外提后第 N 行仍是第 N 行、记号不占行号、Python 同文本不外提）。端到端（假供应商，同一段深层嵌套的 `.rs` 与 `.py`）：`tool output compression: 1237 characters kept out of the conversation`，Rust 结果带 `// begin: 8 spaces omitted` 记号、Python 结果逐行原样。真实代码实测（本仓 85 万字符 rs/ts/tsx/json）：按块外提省 52,358 字符（**6.2%**）、227/258 个窗口命中；对照实验：整窗外提 2.6%、按等缩进段分块 0.9%——数据选了按最小缩进成块。
